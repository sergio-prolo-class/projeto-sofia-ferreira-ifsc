...
void adicionarReceitaInicio(Receita *receitas, int *totalReceitas, char *nome, char *ingredientes, char *preparo, int porcoes);
...
int main(){
    ...
    do {
        ...
        printf("6. Adicionar nova receita em 1°\n");
        ...
        switch (opcao){
            ...
            case 6: 
                ...
                adicionarReceitaInicio(receitas, &totalReceitas, nome, ingredientes, preparo, porcoes);
                printf("Receita adicionada em 1° lugar com sucesso!\n");
                break;
            case 7: 
...
// Adicionar uma receita no inicio da lista
void adicionarReceitaInicio(Receita *receitas, int *totalReceitas, char *nome, char *ingredientes, char *preparo, int porcoes){
    if(*totalReceitas >= MAXRECEITAS) return;
    for(int i = *totalReceitas; i > 0; i--){
        receitas[i] = receitas[i - 1];
    }
    strcpy(receitas[0].nome, nome);
    strcpy(receitas[0].ingredientes, ingredientes);
    strcpy(receitas[0].preparo, preparo);
    receitas[0].porcoes = porcoes;
